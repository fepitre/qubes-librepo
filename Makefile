...
FEDORA_SOURCES := https://src.fedoraproject.org/rpms/librepo/raw/f$(subst fc,,$(DIST))/f/sources
SRC_FILE := librepo-$(VERSION).tar.gz

BUILDER_DIR ?= ../..
SRC_DIR ?= qubes-src

DISTFILES_MIRROR ?= https://github.com/rpm-software-management/librepo/archive/$(VERSION)/librepo-$(VERSION)/
